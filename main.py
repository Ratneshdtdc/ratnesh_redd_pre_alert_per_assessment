...
col1, col2 = st.columns(2)
col1.metric("Current Avg REDD Today or earlier Pending Volume", int(final['Critical_Pending'].sum()/11))
col2.metric("Targeted REDD Today or earlier Pending Volume", int(final['Target_Critical_Pending'].sum()/11),str(round(((final['Target_Critical_Pending'].sum()/final['Critical_Pending'].sum())-1)*100,2))+"%" )
